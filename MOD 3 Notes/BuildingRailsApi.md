...
!. Dealing with CORS (Cross-Origin Resource Sharing) 
    ...
    3. uncomment the following to the 'cors.rb' inside of the 'class Application < Rails::Application'
   
            config.middleware.insert_before 0, Rack::Cors do
                allow do
                    origins '*' (this is the url that we allow through)
                    resource '*'(This gives access to specific resource), headers: :any, methods: [:get, :post]
                end
            end
